...
namespace GameServer.Networking
{
    ...
    public class Packet : IDisposable
    {
        ...
        /// <param name="_value">The Vector2Int to add.</param>
        public void Write(Vector2Int _value)
        {
            Write(_value.X);
            Write(_value.Y);
        }
        /// <summary>Adds a TroopTemplate to the packet.</summary>
        ...
        /// <summary>Reads a TroopTemplate from the packet.</summary>
        /// <param name="_moveReadPos">Whether or not to move the buffer's read position.</param>
        public TroopTemplate ReadTroopTemplate(bool _moveReadPos = true)
        {
            PlayerId controllingPlayer = (PlayerId)ReadInt(_moveReadPos);
            int health = ReadInt(_moveReadPos);
            int movePoints = ReadInt(_moveReadPos);
            int orientation = ReadInt(_moveReadPos);

            return new TroopTemplate(controllingPlayer, movePoints, health, orientation);
        }
